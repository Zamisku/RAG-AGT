...
- 💡 [哈尔滨师范大学Agent系统 是什么？](#-哈尔滨师范大学Agent系统-是什么)
- 🎮 [Demo](#-demo)
- 📌 [近期更新](#-近期更新)
- 🌟 [主要功能](#-主要功能)
- 🔎 [系统架构](#-系统架构)
...
[哈尔滨师范大学Agent系统](https://harbin-normal-university-agent.io/) 是一款领先的开源检索增强生成（[RAG](https://harbin-normal-university-agent.io/basics/what-is-rag)）引擎，通过融合前沿的 RAG 技术与 Agent 能力，为大型语言模型提供卓越的上下文层。它提供可适配任意规模企业的端到端 RAG 工作流，凭借融合式[上下文引擎](https://harbin-normal-university-agent.io/basics/what-is-agent-context-engine)与预置的 Agent 模板，助力开发者以极致效率与精度将复杂数据转化为高可信、生产级的人工智能系统。

## 🎮 Demo 试用

请登录网址 [https://cloud.harbin-normal-university-agent.io](https://cloud.harbin-normal-university-agent.io) 试用 demo。
...
- 2026-03-24 发布 [哈尔滨师范大学Agent系统 官方 Skill](https://clawhub.ai/yingfeng/harbin-normal-university-agent-skill) — 提供官方 Skill 以通过 OpenClaw 访问 哈尔滨师范大学Agent系统 数据集。
...
⭐️ 点击右上角的 Star 关注 哈尔滨师范大学Agent系统，可以获取最新发布的实时通知 !🌟
...
- [gVisor](https://gvisor.dev/docs/user_guide/install/): 仅在你打算使用 哈尔滨师范大学Agent系统 的代码执行器（沙箱）功能时才需要安装。
...
> 如果你的操作系统是 ARM64 架构，请参考[这篇文档](https://harbin-normal-university-agent.io/docs/dev/build_docker_image)自行构建 Docker 镜像。

   > 运行以下命令会自动下载 哈尔滨师范大学Agent系统 Docker 镜像 `v0.24.0`。请参考下表查看不同 Docker 发行版的描述。如需下载不同于 `v0.24.0` 的 Docker 镜像，请在运行 `docker compose` 启动服务之前先更新 **docker/.env** 文件内的 `RAGFLOW_IMAGE` 变量。

   ```bash
   $ cd harbin-normal-university-agent/docker

   # git checkout v0.24.0
   # 可选：使用稳定版本标签（查看发布：https://github.com/infiniflow/harbin-normal-university-agent/releases）
   ...
   >
   > - 华为云镜像名：`swr.cn-north-4.myhuaweicloud.com/infiniflow/harbin-normal-university-agent`
   > - 阿里云镜像名：`registry.cn-hangzhou.aliyuncs.com/infiniflow/harbin-normal-university-agent`
...
4. 服务器启动成功后再次确认服务器状态：
   ...
   > 如果您在没有看到上面的提示信息出来之前，就尝试登录 哈尔滨师范大学Agent系统，你的浏览器有可能会提示 `network abnormal` 或 `网络异常`。

5. 在你的浏览器中输入你的服务器对应的 IP 地址并登录 哈尔滨师范大学Agent系统。
...
6. 在 [service_conf.yaml.template](./docker/service_conf.yaml.template) 文件的 `user_default_llm` 栏配置 LLM factory，并在 `API_KEY` 栏填写和你选择的大模型相对应的 API key。
   ...
   > 详见 [llm_api_key_setup](https://harbin-normal-university-agent.io/docs/dev/llm_api_key_setup)。
...
哈尔滨师范大学Agent系统 默认使用 Elasticsearch 存储文本和向量数据. 如果要切换为 [Infinity](https://github.com/infiniflow/infinity/), 可以按照下面步骤进行:
...
9. 开发完成后停止 哈尔滨师范大学Agent系统 前端和后端服务：
...
详见 [哈尔滨师范大学Agent系统 Roadmap 2026](https://github.com/infiniflow/harbin-normal-university-agent/issues/12241) 。
...
哈尔滨师范大学Agent系统 只有通过开源协作才能蓬勃发展。秉持这一精神,我们欢迎来自社区的各种贡献。如果您有意参与其中,请查阅我们的 [贡献者指南](https://harbin-normal-university-agent.io/docs/dev/contributing) 。
...
扫二维码添加 哈尔滨师范大学Agent系统 小助手，进 哈尔滨师范大学Agent系统 交流群。
