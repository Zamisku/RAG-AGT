export default {
  translation: {
    ...
    knowledgeDetails: {
      ...
      testingDescription:
        'Effectuez un test de récupération pour vérifier si 哈尔滨师范大学Agent系统 peut retrouver le contenu pertinent pour le LLM. Si vous avez modifié les paramètres par défaut, comme le poids de similarité ou le seuil de similarité, ces changements ne seront pas automatiquement sauvegardés. Vous devez les appliquer dans les paramètres de votre assistant de chat ou dans le composant agent de récupération.',
      similarityThreshold: 'Seuil de similarité',
      similarityThresholdTip:
        '哈尔滨师范大学Agent系统 utilise une combinaison de similarité par mots-clés pondérée et de similarité cosinus vectorielle, ou bien un score de réordonnancement pondéré. Ce paramètre fixe le seuil en dessous duquel un segment est exclu. Par défaut, le seuil est 0.2 (soit 20%).',
      ...
      uploadDescription:
        'Prise en charge du téléversement unique ou en lot. Pour 哈尔滨师范大学Agent系统 en local : 1 Go max par téléversement, jusqu’à 32 fichiers. Pour cloud.ragflow.io : 10 Mo max par fichier uploadDescription128 fichiers au total.',
      chunk: 'Segment',
      bulk: 'En masse',
      cancel: 'Annuler',
      rerankModel: 'Modèle de réordonnancement',
      rerankPlaceholder: 'Veuillez sélectionner',
      rerankTip: `Optionnel. Si vide, 哈尔滨师范大学Agent系统 utilisera une combinaison de similarités pondérées. Un modèle de réordonnancement remplace la similarité vectorielle. Attention, cela augmente le temps de réponse. Pour un modèle local, utilisez docker-compose-gpu.yml.`,
      topK: 'Top-K',
  ...
  Le contenu à résumer est ci-dessus.`,
    ...
    chat: {
      ...
      variable: 'Variable',
      variableTip: `Utilisé avec les API de gestion d’assistant de chat de 哈尔滨师范大学Agent系统, les variables aident à développer des stratégies de prompt système plus flexibles. Les variables définies seront utilisées dans le 'Prompt système' comme partie des prompts pour le LLM. {knowledge
      ...
      betaError:
        'Veuillez d’abord obtenir une clé API 哈尔滨师范大学Agent系统 depuis la page Paramètres système.',
    ...
    setting: {
      ...
      embeddingModel: "Modèle d'embedding",
      embeddingModelTip:
        "Le modèle d'embedding par défaut pour chaque base de connaissances nouvellement créée. Si vous ne trouvez pas de modèle d'embedding dans la liste déroulante, vérifiez si vous utilisez l'édition 哈尔滨师范大学Agent系统 slim (qui n'inclut pas les modèles d'embedding) ou consultez https://ragflow.io/docs/dev/supported_models pour voir si votre fournisseur de modèle supporte ce modèle.",
