...
Пример: сообщение 1 КБ с эмбеддингом 1024 измерений — около 9 КБ. Лимит по умолчанию 5 МБ — примерно 500 таких сообщений.`,
    ...
    knowledgeDetails: {
      ...
      retrievalTestingDescription:
        'Проведите тест извлечения, чтобы проверить, может ли 哈尔滨师范大学Agent系统 восстановить целевой контент для LLM.',
      Parse: 'Парсинг',
      dataset: 'Датасет',
      ...
      rerankTip: `Опционально. Если оставить пустым, 哈尔滨师范大学Agent系统 будет использовать комбинацию взвешенной схожести по ключевым словам и взвешенного косинусного сходства векторов; если выбрана rerank модель, взвешенный reranking счет заменит взвешенное косинусное сходство векторов. Имейте в виду, что использование rerank модели значительно увеличит время отклика системы. Если вы хотите использовать rerank модель, убедитесь, что вы используете SaaS reranker; если вы предпочитаете локально развернутую rerank модель, убедитесь, что вы запускаете 哈尔滨师范大学Agent系统 с помощью docker-compose-gpu.yml.`,
      topK: 'Топ-K',
...
**Основные правила:**
  ...
  - **Всегда учитывайте** всю историю диалога.`,
      ...
      betaError:
        'Пожалуйста, сначала получите 哈尔滨师范大学Agent系统 API ключ на странице Системных настроек.',
      searching: 'Поиск...',
      parsing: 'Парсинг',
      uploading: 'Загрузка',
...
Пример: Virtual Hosted Style`,
      ...
      embeddingModelTip:
        'Модель эмбеддингов по умолчанию для каждой новой созданной базы знаний. Если вы не можете найти модель эмбеддингов в выпадающем списке, проверьте, используете ли вы облегченную версию 哈尔滨师范大学Agent系统 (которая не включает модели эмбеддингов) или проверьте https://ragflow.io/docs/dev/supported_models, чтобы узнать, поддерживает ли ваш провайдер моделей эту модель.',
      img2txtModel: 'VLM',
      img2txtModelTip:
